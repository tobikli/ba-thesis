...
1.	Develop an automated Theia End-to-End (E2E) testing suite using Playwright
...
==  Develop an automated Theia End-to-End (E2E) testing suite using Playwright
...
The framework models typical user workflows, including editing code, executing programs, and interacting with the terminal to replicate real-world usage patterns. The tests assess latency, response times, and resource consumption under increasing user loads. Furthermore, the tests identify the system's breaking point, where performance degrades or failures occur, to provide insights for potential optimizations. The focus is on developing a scalable and repeatable methodology that evaluates the performance of Theia Cloud for ongoing performance evaluations.
...
A crucial aspect of performance evaluation is understanding how system components behave under load. This thesis uses Grafana and other available monitoring tools to analyze key performance metrics, including CPU and memory usage, response times, and request throughput.
