...
== Status
This section provides an overview of the functional requirements of the testing framework, as presented in @functionalrequirements.
...
1. *Design of a Test Architecture for Cloud-Based IDEs*
 ...
 A modular test architecture was designed to enable automated end-to-end and load testing of online IDEs integrated into complex systems such as Artemis. The design separates the test logic from the system under test, allowing flexible deployment against self-hosted or external environments.
2. *Implementation of a Realistic End-to-End Test Suite*

 We developed a test suite to simulate authentic student workflows, including file editing, compilation, and terminal interaction. These tests validate both the functional correctness and responsiveness of the IDE, providing a foundation for future regression testing.
...
The work further emphasized the importance of balancing automation and manual evaluation when operating within a managed infrastructure. By decoupling heavy load tests from continuous integration pipelines, the testing approach ensures both safety and flexibility for developers.
...
Although integrating an automated pipeline execution was not feasible at the current stage, it is a promising future work to enhance the system's functionality. This would allow for the execution of the tests automatically in the CI/CD pipeline, which would be a significant improvement in the development process. Regarding execution platform and conditions, it would be interesting to evaluate the performance of the tests on different platforms and conditions, such as different browsers, operating systems, and network conditions.
